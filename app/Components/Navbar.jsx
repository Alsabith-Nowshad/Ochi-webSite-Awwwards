...
function Navbar() {
  ...
  return (
    <div className='navbar fixed z-[999] w-full h-[10vh] px-20 flex justify-between items-center'>
      <div className="logo">
        <svg className='w-[40px] lg:w-[70px]' width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="links hidden gap-3 md:flex lg:gap-10">
        {["Services", "Our Works", "About Us", "Insights", "Contact"].map((item, index) => (
          <div key={index} className='flex flex-col relative group'>
            <a
              onClick={() => handleLinkClick(index)}
              className={`font-light text-[13px] ${index === 4 && "ml-32"} lg:text-[15px] cursor-pointer`}
            >
              {item}
            </a>
            {index !== 4 && (
              <span
                className={`absolute bottom-0 ${activeIndex === index ? "scale-x-100" : "scale-x-0"} left-0 w-full h-0.5 bg-[#ff2a2a] origin-left transition-all duration-300 group-hover:scale-x-100`}
              ></span>
            )}
          </div>
        ))}
